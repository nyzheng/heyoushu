(function () {
    ...
    const mobile_JSfn = () => {
        ...
        let ul = document.querySelector(".mobile-slider-container > ul");
        let lis = document.querySelectorAll(".mobile-slider-container > ul > li");
        let dots = document.querySelectorAll(".mobile-slider-container > .mobile-dots_wrapper > .mobile-dot_navigation");

        let control_prev = document.querySelector(".mobile-slider-container > .control_prev");
        let control_next = document.querySelector(".mobile-slider-container > .control_next");

        let imgWidth = document.querySelector(".mobile-slider-container > ul > li").offsetWidth;
        ...
        ul.prepend(document.querySelector(".mobile-slider-container > ul > li:last-child")); // 加入開頭

        ul.style.cssText = `left: ${-imgWidth}px;`;

        const dotsShow = () => {
            for (let item of dots) {
                item.classList.remove("mobile-slider_active_dot");
            }

            dots[active].classList.add("mobile-slider_active_dot");
        };
        ...
        const moveRight = async (index) => {
            ...
            ul.append(document.querySelector(".mobile-slider-container > ul > li:first-child")); // 加入結尾
        ...
        /*
        document.querySelector(".mobile-slider-container").addEventListener("mouseover", (event) => {
            event.preventDefault(); // 取消預設觸發行為
            event.stopPropagation(); // 終止冒泡事件
        ...
        document.querySelector(".mobile-slider-container").addEventListener("mouseleave", (event) => {
            event.preventDefault(); // 取消預設觸發行為
            event.stopPropagation(); // 終止冒泡事件
        ...
        */
        ...
        //小圓按鈕
        var dotElements = document.querySelectorAll('.mobile-dot_navigation');
        var eventIndex = 0;
        dotElements.forEach(function (dotElement) {
            var currentEventIndex = eventIndex;
            dotElement.addEventListener('click', function () {
        ...
        const findActiveDot = () => {
            let activeIndex = -1;
            dotElements.forEach(function (dotElement, index) {
                if (dotElement.classList.contains('mobile-slider_active_dot')) {
                    activeIndex = index;
                }
            });
            return activeIndex
    ...
    window.addEventListener("load", mobile_JSfn, false);
